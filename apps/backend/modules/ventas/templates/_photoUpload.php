<div id="pictures_list" class="sf_admin_form_row">
  <?php include_partial('photoListe', array('list_registros' => Doctrine_Core::getTable('VentasGaleria')->getGaleriaPorVentas($form->getObject()->getId()))) ?>
</div>
  ...
  $("#formulario-video").dialog({
        ...
        buttons: {
            "Agregar": function() {
                ...
                if ( bValid ) {
                    $('#image-loader').show();
                    $.get("<?php echo url_for("@ventas_galeria_upload") ?>", 
                    { 
                        ventas_id: "<?php echo $form->getObject()->getId(); ?>", 
...
<script>
    ...
    var uploader = new qq.FileUploader({
            ...
            action: '<?php echo url_for(@ventas_galeria_upload) ?>',
            params:
                { ventas_id: <?php echo $form->getObject()->getId(); ?>},
            onComplete: function(id, file, responseJson){
                    $.post("<?php echo url_for(@ventas_galeria_ajax_registro_liste,$form->getObject()) ?>",
                    {
                        ventas_id: <?php echo $form->getObject()->getId(); ?>
                    },
