...
<script>
    ...
    function updateRegistroGaleria(Id,Title,Description){
        $('#image-loader').show();
        $("#pictures_list").load('/<?php echo $app_name ?>.php/gventas/updateRegistroGaleria',
            ...
            $("#gallery" ).sortable({
                    ...
                    update: function(){
                     ...
                     var order = $('#gallery').sortable('serialize');
                         //alert(order);
                     $("#pictures_list").load('/<?php echo $app_name ?>.php/ventas/ajaxRegistroOrder?id=<?php echo $list_registros[0]->getVentasId();?>&'+order,
                        {},
                        function(){
                          $('#image-loader').hide();
                        });
                            ...
                            <li id="registro-<?php echo $registro->getId()?>" class="ui-widget-content ui-corner-tr picture">
                                    ...
                                    <div id="image_<?php echo $registro->getId(); ?>">
                                        <!--a href="<?php echo $registro->getArchivoValid()?>" title="<?php echo $registro->getContenido()?>" rel="prettyPhoto[pp_gal]" style="padding: 0px; margin:0px;"-->
                                            <img class="basic" src="<?php echo $registro->getThumbnailValid(); ?>"  title="<?php echo $registro->getTitulo(); ?>" descripcion="<?php echo $registro->getContenido(); ?>" url="<?php echo url_for('ventas_galeria_edit',$registro,true)?>" alt="<?php echo $registro->getPosition()?>" />
                                        <!--/a-->
                                    </div>
                                    <a href="#" onclick="titularizar('<?php echo $registro->getId() ?>')" title="Editar registro" class="ui-icon ui-icon-pencil">Editar registro</a>
                                    <a href="#" onclick="eliminar('<?php echo $registro->getId() ?>','<?php echo url_for('ventas_galeria_ajax_registro_delete', $registro) ?>')" title="Eliminar registro" class="ui-icon ui-icon-trash">Eliminar registro</a>
                            </li>
