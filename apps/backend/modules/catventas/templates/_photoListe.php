...
<script>
    ...
    function saveTitle(Id,Title){
        ...
        $.post('/<?php echo $app_name ?>.php/ventas/updateTitle',
            {id: Id, title: Title},
            function(data){
                $('#image-loader').show();
            ...
            $("#gallery" ).sortable({
                    ...
                    $('#image-loader').show();
                     var order = $('#gallery').sortable('serialize');
                         //alert(order);
                     $.post('/<?php echo $app_name ?>.php/catventas/ajaxRegistroOrder?id=<?php echo $list_registros[0]->getCategoriaId();?>&'+order,
                     {},
                     function(data){
                          $("#pictures_list").html(data);
                          $('#image-loader').hide();
                    ...
                    <?php if($list_registros->count() > 0): ?>
                        ...
                        <?php foreach( $list_registros as $i=>$registro ): ?>
                            ...
                            <li id="registro-<?php echo $registro->getId()?>" class="ui-widget-content ui-corner-tr picture">
                                    ...
                                    <a href="<?php echo url_for('ventas_edit',$registro)?>" title="Editar registro" class="ui-icon ui-icon-pencil">Editar registro</a>
                                    <a href="#" onclick="eliminar('<?php echo $registro->getId() ?>','<?php echo url_for('ventas_ajax_registro_delete', $registro) ?>')" title="Eliminar ventas" class="ui-icon ui-icon-trash">Eliminar ventas</a>
                            </li>
                        ...
                        <div class="clear"></div>
                        <?php else: ?>
                        <p>No hay ventas en la categoria</p>
                        <?php endif; ?>
...
<div id="dialog-confirm" class="eliminable" title="Eliminar ventas">
   <div style="float: right; height: 120px;">
      <img class="image-dialog" src="" id=""/>
   </div>
  <div style="clear: both;"></div>
  <p style="margin-top:20px;">¿Confirmar eliminar la ventas?</p>
