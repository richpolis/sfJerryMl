<div id="pictures_list" class="sf_admin_form_row">
  <?php include_partial('photoListe', array('list_registros' => Doctrine_Core::getTable('TalentoGaleria')->getGaleriaPorPublicacion($form->getObject()->getId()))) ?>
</div>
...
<table>
    <tr>
        ...
        <td>
            ...
            <!--a href="<?php echo url_for('talento_galeria_new',array('publicacion_id'=>$form->getObject()->getId()))?>"-->
  ...
  $("#formulario-video").dialog({
        ...
        buttons: {
            "Agregar": function() {
                ...
                if ( bValid ) {
                    $('#image-loader').show();
                    $.post('<?php echo url_for(@talento_galeria_upload) ?>', 
                    { 
...
<script>
    ...
    var uploader = new qq.FileUploader({
            ...
            action: '<?php echo url_for(@talento_galeria_upload) ?>',
            params:
                { publicacion_id: <?php echo $form->getObject()->getId(); ?>},
            onComplete: function(id, file, responseJson){
                    $.post("<?php echo url_for(@talento_galeria_ajax_registro_liste,$form->getObject()) ?>",
                    {
                        publicacion_id: <?php echo $form->getObject()->getId(); ?>
                    },
