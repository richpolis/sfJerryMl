...
.gallery li { float: left; width: 250px; height: 100px; padding: 0.4em; margin: 0 0.4em 0.4em 0; text-align: center; list-style-type: none; }
.gallery li h5 { margin: 0 0 0.4em; cursor: move; font-size: small; }
.gallery li a { float: right;}
.gallery li a.ui-icon-pencil { float: left; }
.gallery li img {cursor: move; width: 96px; float:left;}
.gallery li .texto-registro {cursor: move; width: 150px;  height:60px; float:left;overflow: hidden;}
</style>
<script>
    ...
    function saveTitle(Id,Title){
        ...
        $.post('/<?php echo $app_name ?>.php/publicaciones_newsletters/updateTitle',
            {id: Id, title: Title},
            function(data){
                $('#image-loader').show();
            ...
            $("#gallery" ).sortable({
                    ...
                    $('#image-loader').show();
                     var order = $('#gallery').sortable('serialize');
                         //alert(order);
                     $.post('/<?php echo $app_name ?>.php/templates_newsletters/ajaxRegistroOrder?id=<?php echo $list_registros[0]->getTemplateId();?>&'+order,
                     {},
                     function(data){
                          $("#pictures_list").html(data);
                          $('#image-loader').hide();
                    ...
                    <?php if($list_registros->count() > 0): ?>
                        ...
                        <?php foreach( $list_registros as $i=>$registro ): ?>
                            ...
                            <li id="registro-<?php echo $registro->getId()?>" class="ui-widget-content ui-corner-tr picture">
                                    ...
                                    <a href="<?php echo url_for('publicaciones_newsletters_edit',$registro)?>" title="Editar registro" class="ui-icon ui-icon-pencil">Editar registro</a>
                                    <a href="#" onclick="eliminar('<?php echo $registro->getId() ?>','<?php echo url_for('publicaciones_newsletters_ajax_registro_delete', $registro) ?>')" title="Eliminar publicacion" class="ui-icon ui-icon-trash">Eliminar Publicacion</a>
                            </li>
