    ...
    <article class="categorias-talento-item">
        ...
        <ul class="ul-<?php echo $categoria->getSlug()?>" style="position:relative;">
            ...
            <li id="<?echo $categoria->getSlug().'-'.$cont?>" class="categorias-talento-item-imagen">
                <img style="cursor:pointer;" src="<?php echo $talento->getRenderGrayscaleImagen(150,150)?>" onclick="$.irAPagina('<?php echo url_for('categoria_talento',$categoria)?>');"/>
            </li>
            <?php $cont++;?>
...
<script>
    ...
    <?php if($categoria->getTalento()->count()>0):?>    
        var semilla<?php echo $key?>=<?php echo $categoria->getTalento()->count();?>;
        registros<?php echo $key?>=[
            <?php foreach($categoria->getTalento() as $talento):?>
             '<?php echo $talento->getRenderGrayscaleImagen(150,150)?>',           
            <?php endforeach;?>
        ];
        ids<?php echo $key?>=[
            <?php $contador=1;?>
            <?php foreach($categoria->getTalento() as $talento):?>
             '<?echo $categoria->getSlug().'-'.$contador?>',
             <?php $contador++;?>
            <?php endforeach;?>
        ];
